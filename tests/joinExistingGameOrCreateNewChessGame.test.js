...
describe("joinExistingGameOrCreateNewChessGame", () => {
  ...
  test("should join existing game when games are available", () => {
    ...
    const availableGamesCallback = mockSocket.on.mock.calls.find(call => call[0] === "availableGames")[1];
    ...
    //verify that it emits joinExistingGame with the first game
    expect(mockSocket.emit).toHaveBeenCalledWith("joinExistingGame", mockAvailableGames[0]);
  });
